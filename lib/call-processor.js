...
CallProcessor.prototype.start = function() {
  this._srf.invite( function( req, res ) {
    ...
    if( -1 !== req.uri.indexOf('.invalid') ) {
      ...
      console.error('banning %s:%s due to unauthorized attempt to %s', req.source_address, req.source_port, req.uri) ;
      if( ban ) {
          iptables.drop({
            chain: chain,
...
CallProcessor.prototype._unhold = function( dlg, dlgOther, ep1, ep2 ) {
  ep1.bridge( ep2, function(err) {
    ...
    console.log('call successfully taken off hold') ;
  }); 
} ;
